...
def main():
    ...
    for protein_name, box, _tokens in pockets:
        ...
        try:
            ...
            print(f"警告: 生成 receptor.pdbqt 失败，跳过该口袋: {protein_name} pocket#{idx}")
            print("原因:", e)
            continue
        except FileNotFoundError as e:
            print(f"警告: receptor.pdbqt 未生成或丢失，跳过该口袋: {protein_name} pocket#{idx}")
            print("原因:", e)
            continue
        ...
        # --- 第3步：Vina（遇到 parse error / ATOM syntax incorrect 时跳过）---
        try:
            run_cmd(cmd)
        except RuntimeError as e:
            msg = str(e)
            print(f"警告: Vina 失败，跳过该口袋: {protein_name} pocket#{idx}")
            print("原因:", msg)
            # 可选：这里可以额外做标记文件，方便后续统计
            # (pocket_out / "FAILED.txt").write_text(msg, encoding="utf-8")
            continue
